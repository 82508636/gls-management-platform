...
WITH demo(position, recipient_code, shipment_date, due_date, parcel_count, weight, payment_status) AS (
    VALUES
        (1, 'DEMO-LIS', DATE '2026-08-01', DATE '2026-08-31', 1,  4.000::numeric, 'PENDING'),
        (2, 'DEMO-POR', DATE '2026-08-05', DATE '2026-09-04', 1, 12.000::numeric, 'PENDING'),
        (3, 'DEMO-MAD', DATE '2026-07-22', DATE '2026-08-21', 1,  8.000::numeric, 'PAID'),
        (4, 'DEMO-BCN', DATE '2026-08-08', DATE '2026-09-07', 2, 32.100::numeric, 'PENDING'),
        (5, 'DEMO-SEV', DATE '2026-07-28', DATE '2026-08-27', 1,  9.000::numeric, 'PAID'),
        (6, 'DEMO-VLC', DATE '2026-08-12', DATE '2026-09-11', 1, 12.000::numeric, 'PENDING')
), available_routes AS (
    SELECT route.*,
           plan.code AS plan_code, plan.version AS plan_version, plan.currency,
           plan.fuel_surcharge_percent, plan.vat_percent,
           ROW_NUMBER() OVER (PARTITION BY route.destination_country ORDER BY route.sort_order, route.code) AS country_position,
           COUNT(*) OVER (PARTITION BY route.destination_country) AS country_count
    FROM pricing_routes route
    JOIN pricing_plans plan ON plan.id = route.pricing_plan_id
    WHERE route.enabled
), calculated AS (
    SELECT demo.*, customer.id AS customer_id, customer.customer_code, customer.shipping_name,
           ...
           route.pricing_plan_id AS plan_id, route.plan_code, route.plan_version,
           route.currency, route.fuel_surcharge_percent, route.vat_percent,
           route.id AS route_id, route.code AS route_code, route.designation AS route_designation,
    ...
    JOIN available_routes route
      ON route.destination_country = recipient.country
     AND route.country_position = ((demo.position - 1) % route.country_count) + 1
    JOIN LATERAL (
...
INSERT INTO shipments (
    ...
    pricing_route_id, route_code, route_designation, shipment_date, due_date,
...
SELECT md5('demo-shipment-' || position)::uuid,
       ...
       route_id, route_code, route_designation, shipment_date, due_date,
