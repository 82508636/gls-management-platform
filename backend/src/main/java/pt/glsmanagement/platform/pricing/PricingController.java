...
class PricingController {
    ...
    @GetMapping("/plans/{id}") PlanDetail detail(@PathVariable UUID id) { return service.detail(id); }
    ...
    @PutMapping("/plans/{id}")
    PlanDetail update(@PathVariable UUID id, @Valid @RequestBody PlanUpdateRequest request, @AuthenticationPrincipal Jwt jwt) {
        return service.update(id, request, actor(jwt));
    }
    @PostMapping("/plans/{id}/routes") @ResponseStatus(HttpStatus.CREATED)
    RouteResponse addRoute(@PathVariable UUID id, @Valid @RequestBody RouteCreateRequest request, @AuthenticationPrincipal Jwt jwt) {
        return service.addRoute(id, request, actor(jwt));
    }
    @PutMapping("/plans/{planId}/routes/{routeId}")
    RouteResponse updateRoute(@PathVariable UUID planId, @PathVariable UUID routeId,
    ...
    PlanDetail status(@PathVariable UUID id, @Valid @RequestBody StatusRequest request, @AuthenticationPrincipal Jwt jwt) {
