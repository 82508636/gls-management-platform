...
class KeycloakAdminClient {
    ...
    private void replacePlatformRoles(String userId, PlatformRole role) {
        var existing = realmRoleMappings(userId).stream().filter(item -> PLATFORM_ROLES.contains(item.name())).toList();
        if (!existing.isEmpty()) {
    ...
    private String token() {
        ...
        if (response == null || response.accessToken() == null) throw new IdentityOperationException("Keycloak token unavailable");
        return response.accessToken();
    }

    private static IdentityUserResponse response(UserRepresentation user, Set<PlatformRole> roles) {
        return new IdentityUserResponse(user.id(), user.username(), user.email(), user.firstName(), user.lastName(), user.enabled(), roles);
    }

    private record TokenResponse(@com.fasterxml.jackson.annotation.JsonProperty("access_token") String accessToken) {}
    private record UserRepresentation(String id, String username, String email, String firstName, String lastName,
