...
class PickupPoint {
    ...
    static PickupPoint create(PickupPointRequest request) {
        var point = new PickupPoint(); point.id = UUID.randomUUID(); point.createdAt = OffsetDateTime.now(ZoneOffset.UTC);
        point.apply(request); return point;
    }

    void update(PickupPointRequest request) { apply(request); }
    ...
    private void apply(PickupPointRequest request) {
        ...
        openSaturday = request.openSaturday(); openSunday = request.openSunday(); active = request.active(); updatedAt = OffsetDateTime.now(ZoneOffset.UTC);
    }
    ...
    boolean openSaturday() { return openSaturday; } boolean openSunday() { return openSunday; } boolean active() { return active; }
    OffsetDateTime createdAt() { return createdAt; } OffsetDateTime updatedAt() { return updatedAt; }
}
