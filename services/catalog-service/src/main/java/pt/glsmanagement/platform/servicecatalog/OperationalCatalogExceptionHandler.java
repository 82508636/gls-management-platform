...
import pt.glsmanagement.platform.customer.CustomerReferenceUnavailableException;
...
class OperationalCatalogExceptionHandler {
    ...
    @ExceptionHandler(CustomerReferenceUnavailableException.class)
    ResponseEntity<ApiError> dependencyUnavailable(CustomerReferenceUnavailableException exception) {
        return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                .body(ApiError.of(HttpStatus.SERVICE_UNAVAILABLE.value(),
                        "Não foi possível validar os clientes selecionados."));
    }
}
