...
class ReferenceCatalogService {
    ...
    private static String normalizeId(String value) { return value.trim().toUpperCase(Locale.ROOT); }
    private static String normalizeDesignation(String value) { return value.trim().replaceAll("\\s+", " "); }
    private static ReferenceCatalogException duplicate() {
        return new ReferenceCatalogException(ReferenceCatalogException.Reason.DUPLICATE);
    }
    private static ReferenceCatalogException notFound() {
        return new ReferenceCatalogException(ReferenceCatalogException.Reason.NOT_FOUND);
    }
}
