...
public class IdentityAccessControl {
    ...
    IdentityAccessControl(IdentityAccessStateRepository repository) { this.repository = repository; }
    ...
    void disable(String userId) {
        var state = repository.findById(userId)
                .orElseGet(() -> IdentityAccessState.active(userId, PlatformRole.CUSTOMER));
        state.disable();
        repository.save(state);
    }
